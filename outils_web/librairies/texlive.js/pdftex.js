var PDFTeX = function (opt_workerPath) {
  ...
  var determineChunkSize = function () {
    ...
    while (Math.abs(delta) > 100) {
      if (success) {
        min = size;
        if (typeof max === "undefined") delta = size;
        else delta = (max - size) / 2;
      } else {
        max = size;
        if (typeof min === "undefined") delta = (-1 * size) / 2;
        else delta = (-1 * (size - min)) / 2;
      }
      size += delta;
  ...
  self.compile = function (source_code) {
    ...
    self.compileRaw(source_code).then(function (binary_pdf) {
      ...
      pdf_dataurl =
        "data:application/pdf;charset=binary;base64," + window.btoa(binary_pdf);
  ...
  self.compileRaw = function (source_code) {
    ...
    var commands;
    if (initialized) commands = [curry(self, "FS_unlink", ["/input.tex"])];
    else
      commands = [
        curry(self, "FS_createDataFile", [
          "/",
          "input.tex",
          source_code,
          true,
        ...
        curry(self, "FS_createLazyFilesFromList", [
          "/",
          "texlive.lst",
          "./texlive",
          true,
